...
func handle(conn net.Conn) {
	err := conn.SetDeadline(time.Now().Add(10 * time.Second)) //sets the connect to expire after 10 seconds.
	if err != nil {
	...
	for scanner.Scan() {              //func (s *Scanner) Scan() bool; Scan advances the Scanner to the next token, which will then be available through the Bytes or Text method. It returns false when the scan stops,
		...
		fmt.Println(ln)
		fmt.Fprintf(conn, "I heard you say: %s\n", ln) //Fprintf formats according to a forma spacifier and writes to w. It returns the number of bites written and any write error encountered.
